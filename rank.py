...
def main(teams_directory):
    ...
    Game.winner_bonus = 5
    Game.adj_margin_max = 38
    Game.non_fbs_bonus = 21
    Team.ignore_all_non_fbs = False
    Team.ignore_wins_vs_non_fbs = False

    fbs_seen: Set[Team] = read_teams_pages.main(teams_directory)

    rank1 = sorted(list(fbs_seen), key=lambda Team: Team.get_avg_game_metric(0,0,win_factor=10,loss_factor=10,opp_strength_weight=.5,recency_bias=0.05,exclude_team_result_from_opp=True), reverse=True)
    for i, team in enumerate(rank1):
        print(f"{i+1}. {team.get_name()} ({team.get_avg_game_metric(0,0,win_factor=10,loss_factor=10,opp_strength_weight=.5,recency_bias=0.05,exclude_team_result_from_opp=True)})")
    ...
    compare_rankings(read_ranking("rankings/2022-week7.txt"), rank1)
